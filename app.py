                        ...
                        if status == "pending":
                            ...
                            with col1:
                                if st.button("📦 Создать товар", key=f"create_{r[0]}"):
                                    st.session_state[f"show_create_{r[0]}"] = True
                            with col2:
                                if st.button("🗑️ Удалить", key=f"del_{r[0]}"):
                                    delete_request(r[0])
                                    st.rerun()
                            
                            if st.session_state.get(f"show_create_{r[0]}"):
                                with st.form(f"create_form_{r[0]}"):
                                    rooms = get_room_names()
                                    if rooms:
                                        ...
                                        if st.form_submit_button("💾 Сохранить и удалить заявку"):
                                            if new_loc:
                                                create_item_from_request(r[0], r[1], new_loc, new_room, r[2], r[3])
                                                st.session_state[f"show_create_{r[0]}"] = False
