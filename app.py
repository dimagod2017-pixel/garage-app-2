...
def check_and_notify_low_stock():
    ...
    if low_items:
        ...
        if success:
            # Создаем файл-маркер, чтобы не отправлять повторно сегодня
...
def send_daily_report():
    ...
    success, msg = send_email(subject, body)
    if success:
        # Создаем файл-маркер для отчета
...
def check_daily_report():
    """
    Проверяет, отправлялся ли отчет сегодня
    """
    today = datetime.now().strftime("%Y-%m-%d")
    if not os.path.exists(f"report_{today}.txt"):
        return send_daily_report()
    return True, "Отчет уже отправлен сегодня"
...
if role == "admin":
    # Проверка низких остатков (не чаще раза в день)
    today = datetime.now().strftime("%Y-%m-%d")
    if not os.path.exists(f"notified_{today}.txt"):
        ...
        if low_items:
            ...
            if success:
                st.success("📧 Уведомление о низких остатках отправлено на почту!")
            else:
                st.warning(f"⚠️ Не удалось отправить уведомление: {msg}")
    
    # Ежедневный отчет
    if not os.path.exists(f"report_{today}.txt"):
        success, msg = check_daily_report()
        if success:
            st.success("📊 Ежедневный отчет отправлен на почту!")
        else:
            st.warning(f"⚠️ Не удалось отправить отчет: {msg}")
...
with st.sidebar:
    ...
    if role == "admin":
        ...
        if low_items:
            ...
            if st.button("✅ Проверить остатки", use_container_width=True):
                        ...
                        with st.container(border=True):
                            ...
                            with col2:
                                ...
                                for eq in equipment_list:
                                    eq_name_display = eq[1] + (f" ({eq[2]})" if eq[2] else "")
                                    search_options.append(eq_name_display)
                                    units = get_units(eq[0])
                                    for unit_item in units:
                                        search_options.append(f"{eq_name_display} → {unit_item[1]}")
                                ...
                                if filtered_eq:
                                    ...
                                    if selected_eq == "Другое":
                                        object_name_input = st.text_input("Введите название*", key=f"take_custom_{item_id}")
                                    else:
                                        object_name_input = selected_eq
                                else:
                            ...
                            with st.container(border=True):
                                ...
                                with col2:
                                    ...
                                    if filtered_eq:
                                        ...
                                        if selected_eq == "Другое":
                                            object_name_input = st.text_input("Введите название объекта*", key=f"custom_obj_{item_id}")
                                        else:
                                            object_name_input = selected_eq
                                    else:
                                        st.warning("Ничего не найдено")
                                ...
                                with col1:
                                    if st.button("✅ Списать", key=f"save_cons_{item_id}"):
                                        if consume_qty <= 0:
                                            ...
                                            success, message = consume_item(item_id, consume_qty, object_name_input, user, note, "", "confirmed")
                                            if success:
                                                st.success(message)
