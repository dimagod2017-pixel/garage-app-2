    ...
    if role == "admin":
        ...
        if pending:
            if st.sidebar.button(f"📝 Новые заявки: {pending}", key="side_pending", use_container_width=True):
                ...
                st.rerun()
        if returned:
            if st.sidebar.button(f"🔄 Возвраты: {returned}", key="side_returned", use_container_width=True):
                st.session_state.active_tab = 3
                st.rerun()
        if low:
        ...
        if approved:
            if st.sidebar.button(f"✅ Одобрено: {len(approved)}", key="side_approved", use_container_width=True):
                st.session_state.active_tab = 3
                st.rerun()
        if suggested:
            if st.sidebar.button(f"💡 Предложено: {len(suggested)}", key="side_suggested", use_container_width=True):
                st.session_state.active_tab = 3
                st.rerun()
        ...
        with st.expander(f"📝 Новые заявки ({len(pending_reqs)})", expanded=True):
            for req in pending_reqs:
                ...
                with col2:
                    st.caption(r['date'])
                with col3:
                    if st.button("🔍 К заявкам", key=f"goto_pending_{r['id']}"):
                        st.session_state.active_tab = 3
                        st.rerun()
    
    returned_reqs = get_requests(status='returned')
    if returned_reqs:
        with st.expander(f"🔄 Возвращенные ({len(returned_reqs)})", expanded=True):
            for req in returned_reqs:
                ...
                with col2:
                    st.caption(r['date'])
                with col3:
                    if st.button("🔍 К заявкам", key=f"goto_ret_{r['id']}"):
                        st.session_state.active_tab = 3
                        st.rerun()
    ...
    if low_items:
        with st.expander(f"⚠️ Заканчиваются ({len(low_items)})", expanded=True):
            for item in low_items[:5]:
                ...
                with col3:
                    if st.button("📦 К поиску", key=f"goto_low_{item[0]}"):
                        st.session_state.active_tab = 0
                        st.session_state.show_low_stock = True
                        st.rerun()
        ...
        with st.expander(f"✅ Одобренные заявки ({len(approved_list)})", expanded=True):
            for req in approved_list:
                ...
                with col2:
                    st.caption(r['date'])
                with col3:
                    if st.button("👁️ К заявкам", key=f"goto_app_{r['id']}"):
                        st.session_state.active_tab = 3
                        st.rerun()
        ...
        with st.expander(f"💡 Предложенные товары ({len(suggested_list)})", expanded=True):
            for req in suggested_list:
                ...
                with col2:
                    st.caption(r['date'])
                with col3:
                    if st.button("👁️ К заявкам", key=f"goto_sug_{r['id']}"):
                        st.session_state.active_tab = 3
                        st.rerun()
