...
with tabs[1]:
    ...
    # Показываем предупреждения о ТО (если есть)
    to_alerts = get_to_status()
    if to_alerts:
        with st.container():
            st.warning("⚠️ Внимание! Требуется техническое обслуживание:")
            for eq_name, status, msg in to_alerts:
                if status == "overdue":
                    st.error(f"🔴 {eq_name}: {msg}")
                else:
        ...
        with tab2:
            ...
            if maint_records:
                df_journal = pd.DataFrame(maint_records, columns=[
                    "ID", "Техника", "Дата", "След. ТО", "Описание", "Тип", "to_type", "Моточасы", "Кто", "Создано"
                ])
